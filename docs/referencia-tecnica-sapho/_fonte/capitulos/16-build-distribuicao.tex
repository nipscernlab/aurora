...
O \file{download-yanc.js} baixa \file{yanc-bin-v5.4.zip} do \repo{yanc} e o
extrai em \path{components/} (de modo que \path{bin/cmmcomp.exe} etc.\ caiam em
\path{components/bin/}). O \texttt{.zip} contém sete binários:
\texttt{cmmcomp.exe}, \texttt{cppcomp.exe}, \texttt{asmcomp.exe},
\texttt{cpppp.exe}, \texttt{appcomp.exe} e \texttt{comp2gtkw.exe} desde a v4,
mais o \texttt{gen\_gtkw.exe} desde a v5.0, além das
...
\begin{lstlisting}
  ...
  |    |-- download-yanc (yanc-bin-v5.4.zip)       -> components/  |
