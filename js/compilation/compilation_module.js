...
validateVerilogOnlyConfig() {
    ...
    // Testbench is OPTIONAL in Verilog-only mode. The user may want to compile
    // a single .v (e.g., a simple AND gate) without writing a testbench.
    // When absent, iverilog uses the synthesizable top-level as -s.
...
/**
 * Build a GTKWave `.gtkw` save-file that lists the signals at the
 * testbench module's scope so the wave window opens with traces
 * already laid out — no manual drag-from-SST needed for the first run.
 *
 * The generated file is intentionally minimal: dumpfile pointer + flat
 * list of top-scope signals, hex format. GTKWave fills the rest from
 * its defaults. When the Wave Configuration picker lands (Phase 2),
 * this same function will receive an explicit signal list instead of
 * defaulting to the testbench scope.
 */
async generateGtkwForVcd(vcdPath, gtkwPath, tbModule) {
    ...
    const top = scopes.find((s) => s.name === tbModule) || scopes[0];
    if (!top || top.signals.length === 0) return false;

    const slashed = (p) => p.replace(/\\/g, '/');
    const lines = [
        '[*]',
        '[*] Generated by Aurora — auto-listing testbench scope signals',
        '[*]',
    ...
    for (const sig of top.signals) {
        const ref = sig.range
            ? `${top.name}.${sig.name}[${sig.range}]`
            : `${top.name}.${sig.name}`;
        lines.push(ref);
    }
 ...
 *     `$dumpfile("<tb>.vcd"); $dumpvars(1, <tb>);` block injected
 *     before the last `endmodule`. Depth 1 = signals at the
 *     testbench-module scope only; sub-module signals stay
 *     undumped to keep the VCD lean by default. The picker UI in
 *     a follow-up commit will replace this with user-selected
 *     signals.
 *
 ...
 */
async instrumentVerilogOnlyTestbench(testbenchPath, tbModule, tempBaseDir) {
...
// any. Depth 1 covers signals at the testbench module scope; switch
// to depth 0 (or list specific scopes) once the Wave Configuration
// picker is wired up.
initial begin
...
/**
 * Verilog-only compile (no processors).
 *
...
async runVerilogOnlyGtkWave() {
    ...
    try {
        ...
        if (!gtkwSaveFile) {
            ...
            try {
                const wrote = await this.generateGtkwForVcd(vcdFile, autoGtkw, simTopModule);
                if (wrote) {
                    gtkwSaveFile = autoGtkw;
                    this.terminalManager.appendToTerminal('twave',
                        'Auto-generated GTKWave layout from testbench scope', 'info');
                }
            } catch (genErr) {
                this.terminalManager.appendToTerminal('twave',
