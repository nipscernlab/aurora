...
\section{Plano: repositório de bibliotecas sob demanda}

A ideia é preencher o vazio da Seção 4: manter um repositório de bibliotecas
Python interessantes e deixar o usuário baixar sob demanda, instalando-as no
runtime embarcado. As bibliotecas especiais que não empacotarmos ficam a cargo do
Python do próprio usuário, via terminal TCMD.

\subsection{A restrição técnica que orienta tudo}

O nosso Python é um build MinGW do MSYS2, não o CPython oficial (MSVC) da
python.org. Isso tem uma consequência prática forte:

\begin{itemize}
  \item Bibliotecas Python puras (só \texttt{.py}, sem extensão em C) funcionam
        em qualquer interpretador, então basta colocá-las no
        \texttt{site-packages}.
  \item Bibliotecas com extensão em C (\texttt{numpy}, \texttt{scipy},
        \texttt{pandas}, etc.) são compiladas para uma ABI específica. As wheels
        binárias da PyPI são feitas para o CPython MSVC e em geral NÃO carregam no
        Python MinGW. Para essas, o caminho correto é usar os pacotes mingw do
        próprio MSYS2 (por exemplo \texttt{mingw-w64-x86\_64-python-numpy}), que
        são compilados para a mesma ABI do nosso Python.
\end{itemize}

Em resumo: um repositório de wheels da PyPI não serve direto para o nosso Python.
O repositório precisa guardar ou libs puras, ou pacotes mingw compatíveis.

\subsection{Como instalar sob demanda}

Como o bundle não traz \texttt{pip} nem \texttt{ensurepip} (Seção 4), há três
abordagens possíveis, da mais simples à mais completa:

\begin{itemize}
  \item Guardar no repositório os pacotes já prontos (a árvore de arquivos da
        biblioteca) e, no download, extrair direto no
        \texttt{site-packages} do bundle. Funciona bem para libs puras e para
        pacotes mingw pré-extraídos, sem precisar de gerenciador de pacotes.
  \item Adicionar \texttt{pip} ao runtime (via \texttt{ensurepip} ou um
        \texttt{get-pip.py}) e então instalar com \texttt{pip install}. Serve para
        libs puras; para libs com C, esbarra na restrição de ABI acima.
  \item Para as libs com C, distribuir os pacotes mingw do MSYS2 e colocá-los no
        lugar, já compatíveis com o nosso Python.
\end{itemize}

A escolha entre elas depende de quão longe queremos ir com libs binárias. Para
começar, guardar os pacotes prontos no repositório e extrair no
\texttt{site-packages} é o caminho de menor atrito.
