...
function zeroExtend(bits, width) {
...
export class ComplexVcdScanner {
    ...
    buf = '';
    inBody = false;
    idWidth = new Map(); // id VCD -> largura (so complexos)
    values = new Set();
    capped = false;
    constructor(maxValues = 16384) {
        this.maxValues = maxValues;
    }
    feed(chunk) {
        if (this.capped || !chunk)
            return;
        this.buf += chunk;
        let i;
 ...
 */
export function buildComplexMapping(name, decodedByValue) {
