...
function toRelativeIfInside(absPath, baseDir) {
    if (!absPath || !baseDir || !isAbsolutePath(absPath))
        return absPath;
    ...
    if (!normalizedAbs.toLowerCase().startsWith(baseWithSep))
        return absPath;
...
function toAbsoluteFromBase(maybeRel, baseDir) {
    if (!maybeRel || isAbsolutePath(maybeRel))
        return maybeRel;
    if (!baseDir)
        return maybeRel;
...
function expandStructurePaths(structure, baseDir) {
    if (!structure || !baseDir)
        return;
    for (const k of PATH_FIELDS_SCALAR) {
        if (structure[k])
            structure[k] = toAbsoluteFromBase(structure[k], baseDir);
    }
    for (const k of PATH_FIELDS_ARRAY) {
        ...
        for (const entry of arr) {
            if (entry && entry.path)
                entry.path = toAbsoluteFromBase(entry.path, baseDir);
        }
    }
}
...
function contractStructurePaths(structure, baseDir) {
    ...
    for (const k of PATH_FIELDS_SCALAR) {
        if (cloned[k])
            cloned[k] = toRelativeIfInside(cloned[k], baseDir);
    }
    for (const k of PATH_FIELDS_ARRAY) {
        const arr = cloned[k];
        if (!Array.isArray(arr))
        ...
        for (const entry of arr) {
            if (entry && entry.path)
                entry.path = toRelativeIfInside(entry.path, baseDir);
        }
    }
    return cloned;
}
