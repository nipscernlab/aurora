...
/** A top-level port read from the Verilator JSON AST. */
export interface VerilatorPort {
  name: string;
...
/** One-hot input wiring read from the <proc>.v YANC_SIM_VIS block. */
...
/** One-hot output wiring read from the <proc>.v YANC_SIM_VIS block. */
...
/** A node of the Verilator JSON AST (only the fields this parser reads). */
interface AstNode {
  addr?: string;
  range?: string;
  dtypep?: string;
 ...
 *     (ausente = 1 bit).
 *
 * @param tree  JSON.parse de V<top>.tree.json
 */
export function parseVerilatorPorts(tree: unknown): VerilatorPort[] {
  const byAddr: Record<string, AstNode> = Object.create(null);
  (function index(node: unknown): void {
    if (Array.isArray(node)) { node.forEach(index); return; }
    if (node && typeof node === 'object') {
      const n = node as AstNode;
      if (typeof n.addr === 'string' && !(n.addr in byAddr)) byAddr[n.addr] = n;
      for (const v of Object.values(node)) index(v);
    }
  })(tree);

  function widthOf(dtypeAddr: string | undefined): number {
    let addr = dtypeAddr;
  ...
  function signedOf(dtypeAddr: string | undefined): boolean {
    let addr = dtypeAddr;
    for (let hops = 0; addr && hops < 8; hops++) {
      const n = byAddr[addr];
      if (!n) break;
  ...
  const ports: VerilatorPort[] = [];
  const seen = new Set<string>();
  (function collect(node: unknown): void {
    if (Array.isArray(node)) { node.forEach(collect); return; }
    if (node && typeof node === 'object') {
      const n = node as AstNode;
      if (n.type === 'VAR' && n.isPrimaryIO === true &&
          ['INPUT', 'OUTPUT', 'INOUT'].includes(n.direction as string) &&
          n.name && !seen.has(n.name)) {
        seen.add(n.name);
        const width = widthOf(n.dtypep);
        ports.push({
          name: n.name,
          direction: (n.direction as string).toLowerCase() as 'input' | 'output' | 'inout',
          width,
          words: Math.max(1, Math.ceil(width / 32)),
          signed: signedOf(n.dtypep),
        });
      }
 ...
 *
 * @param src  conteudo do <proc>.v
 */
export function parseProcessorIO(src: string): { inputs: ProcInput[]; outputs: ProcOutput[] } {
  const s = String(src || '');

  const inputs: ProcInput[] = [];
...
function findPort(ports: VerilatorPort[], name: string): VerilatorPort | null {
  return ports.find((p) => p.name.toLowerCase() === name.toLowerCase()) || null;
}

/** Result of {@link generateVerilatorProcTb}. */
export interface GenProcTbResult {
  source: string;
  hasItr: boolean;
  inputs: ProcInput[];
  outputs: ProcOutput[];
...
/** Input for {@link generateVerilatorProcTb}. */
export interface GenProcTbInput {
  topModule: string;
  /** parseVerilatorPorts(V<top>.tree.json) */
  ports: VerilatorPort[];
  /** parseProcessorIO().inputs */
  inputs: ProcInput[];
  /** parseProcessorIO().outputs */
  outputs: ProcOutput[];
