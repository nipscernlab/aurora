...
export function parseVerilatorPorts(tree) {
    ...
    (function index(node) {
        ...
        if (node && typeof node === 'object') {
            const n = node;
            if (typeof n.addr === 'string' && !(n.addr in byAddr))
                byAddr[n.addr] = n;
    ...
    (function collect(node) {
        ...
        if (node && typeof node === 'object') {
            ...
            if (n.type === 'VAR' && n.isPrimaryIO === true &&
                ['INPUT', 'OUTPUT', 'INOUT'].includes(n.direction) &&
                n.name && !seen.has(n.name)) {
                seen.add(n.name);
                const width = widthOf(n.dtypep);
                ports.push({
                    name: n.name,
                    direction: n.direction.toLowerCase(),
                    width,
...
export function generateVerilatorProcTb({ topModule, ports, inputs, outputs, numClocks = 2000 }) {
    ...
    if (inputs.length > 0) {
        if (!inBus)
            missing.push('in (wiring declara inputs)');
        if (!reqBus)
            missing.push('req_in (wiring declara inputs)');
    }
    if (outputs.length > 0) {
        if (!outBus)
            missing.push('out (wiring declara outputs)');
        if (!enBus)
            missing.push('out_en (wiring declara outputs)');
    }
    if (missing.length) {
    ...
    const tooWide = [];
    if (inputs.length > 0 && inBus && inBus.width > 64)
        tooWide.push(`in (${inBus.width} bits)`);
    if (outputs.length > 0 && outBus && outBus.width > 64)
        tooWide.push(`out (${outBus.width} bits)`);
    if (tooWide.length) {
        throw new Error(`barramento(s) de I/O > 64 bits nao suportado(s) pelo harness Verilator: ` +
    ...
    for (const p of inputs)
        L.push(`  FILE* f_in_${p.port} = fopen("${p.file}", "r");`);
    for (const p of outputs)
        L.push(`  FILE* o_out_${p.port} = fopen("${p.file}", "w");`);
    ...
    for (const p of inputs)
        L.push(`  if(f_in_${p.port}) fclose(f_in_${p.port});`);
    for (const p of outputs)
        L.push(`  if(o_out_${p.port}) fclose(o_out_${p.port});`);
