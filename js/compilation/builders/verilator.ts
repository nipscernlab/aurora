...
const verilatorThreadsArgs = (): string[] =>
  VERILATOR_SIM_THREADS > 0 ? ['--threads', String(VERILATOR_SIM_THREADS)] : [];

export interface VerilatorBuildBuilderCtx {
  perlExe: string;
  ...
  sourceFiles: string[];
  cwd: string;
  /** e.g. ['-Wno-fatal', '-Wno-TIMESCALEMOD'] */
  extraWarnings?: string[];
  /**
   * default true (fluxo Wave). false = sem --trace-fst/--no-trace-top, pro
   * Fast Sim (roda sem gerar onda).
   */
  trace?: boolean;
}

export function buildVerilatorBuildSpec(ctx: VerilatorBuildBuilderCtx): CommandSpec {
...
export interface VerilatorRunBuilderCtx {
  exePath: string;
  /** components/Temp */
  cwd: string;
  mingwBin: string;
 ...
 */
export function buildVerilatorRunSpec(ctx: VerilatorRunBuilderCtx): CommandSpec {
  return {
    step: 'verilator-run',
    binary: ctx.exePath,
    args: [],
...
const TL_WARNINGS: readonly string[] = Object.freeze([
  '-Wno-fatal',
  '-Wno-TIMESCALEMOD',
  '-Wno-DECLFILENAME',
  '-Wno-STMTDLY',
...
export interface VerilatorTbBuildBuilderCtx {
  perlExe: string;
  verilatorScript: string;
  mingwBin: string;
  usrBin: string;
 ...
 */
export function buildVerilatorJsonSpec(ctx: VerilatorTbBuildBuilderCtx): CommandSpec {
 ...
 */
export function buildVerilatorTbBuildSpec(ctx: VerilatorTbBuildBuilderCtx): CommandSpec {
  const warnings = ctx.extraWarnings || TL_WARNINGS;
  return {
...
export interface VerilatorTbRunBuilderCtx {
  exePath: string;
  /** diretorio de I/O (.in/.out vivem aqui) */
  cwd: string;
  mingwBin: string;
  usrBin: string;
  /** teto de seguranca (+cycles=N); a sim normalmente termina antes, no EOF da 1a entrada */
  cycles?: number;
}
 ...
 */
export function buildVerilatorTbRunSpec(ctx: VerilatorTbRunBuilderCtx): CommandSpec {
  const args: string[] = [];
