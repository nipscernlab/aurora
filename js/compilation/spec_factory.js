...
import { buildCmmSpec, buildAsmPreSpec, buildAsmSpec, buildIverilogCheckSpec, buildIverilogBuildSpec, buildVvpRunSpec, buildCocotbRunSpec, buildVerilatorBuildSpec, buildVerilatorRunSpec, buildFst2VcdSpec, buildGtkwaveSpec, buildYosysHierarchySpec, } from './builders/index.js';
...
async function findProcessor(processorName) {
    ...
    const processors = (structure.processors || []);
...
export async function buildSpecForStep(step, processorName) {
    ...
    const synth = (structure.synthesizableFiles || [])
        .map((f) => (typeof f === 'string' ? f : f?.path))
        .filter((p) => Boolean(p));
    const tbFiles = structure.testbenchFiles;
    ...
    if (step === 'cocotb-run') {
        if (!tb || !tbIsPython)
            throw new Error('cocotb-run needs a Python .py testbench');
        if (!topLevelFile || !topLevelModuleName)
            throw new Error('cocotb-run needs a Verilog top-level set');
        const pyStatus = await window.electronAPI.getPythonStatus();
        if (!pyStatus?.ok)
            throw new Error('Python was not found');
        if (!pyStatus.isBundled)
            throw new Error('cocotb-run requires Aurora bundled Python');
        if (!pyStatus.hasCocotb)
            throw new Error('Aurora bundled Python is missing cocotb');
        ...
        const pythonHome = await window.electronAPI.dirname(await window.electronAPI.dirname(pyStatus.pythonPath));
    ...
    if (step === 'iverilog-build') {
        if (tbIsPython)
            throw new Error('iverilog-build cannot use a Python testbench; use cocotb-run');
        if (!simTopModule)
            throw new Error('iverilog-build needs a testbench or top-level set');
    ...
    if (step === 'vvp-run') {
        if (tbIsPython)
            throw new Error(`${step} cannot use a Python testbench; use cocotb-run`);
        if (!simTopModule)
            throw new Error(`${step} needs a testbench`);
    ...
    if (step === 'verilator-build' || step === 'verilator-run') {
        ...
        if (step === 'verilator-build') {
            if (!simTopModule)
                throw new Error('verilator-build needs a testbench or top-level set');
    ...
    if (step === 'fst2vcd') {
        if (!simTopModule)
            throw new Error('fst2vcd needs a testbench');
    ...
    if (step === 'gtkwave') {
        if (!simTopModule)
            throw new Error('gtkwave needs a testbench');
