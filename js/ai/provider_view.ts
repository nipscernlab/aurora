// provider_view.ts: pure provider/model view helpers for the AI assistant,
...
import type { EntradaDeProvedor, MetaDoProvedor, Opcao } from './ai_metadata.js';
...
export function providerOptionsHtml(providers: EntradaDeProvedor[], currentProvider: string | null): string {
    return providers.map((p) => {
        const meta: Partial<MetaDoProvedor> = PROVIDER_META[p.name] || { label: p.name };
...
export function modelPresetsHtml(models: Opcao[], active: string): string {
    return models.map((m) =>
...
export function faithfulModelName(entry: EntradaDeProvedor | null | undefined, provider: string | null): string {
    const model = entry?.model || '';
    const sm = SUB_META[provider as string];
    if (sm) {
