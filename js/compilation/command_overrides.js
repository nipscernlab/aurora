/**
 * command_overrides.js — registry of pending and persisted command
...
export async function resolveOverride(step, processorName) {
  ...
  const tryLayer = (key, layer, source) => {
    const ov = layer.get ? layer.get(key) : layer[key];
    if (ov) {
 ...
 * @param {{consumeEphemeral?: boolean}} [options]
 */
...
export async function setOverride({ step, processorName = null, override, persist = false, note }) {
  ...
  const cleaned = {
    ...
    scope: persist ? 'persisted' : 'ephemeral',
  };
  ...
  return { key, scope: cleaned.scope };
}
