...
function logOverride(spec, baseSpec, override, sources, channel) {
    ...
    if (diff.added.length)
        parts.push(`+args: ${diff.added.join(' ')}`);
    if (diff.removed.length)
        parts.push(`-args: ${diff.removed.join(' ')}`);
    for (const [k, v] of Object.entries(diff.envAdded))
        parts.push(`+env: ${k}=${v}`);
    for (const k of diff.envRemoved)
        parts.push(`-env: ${k}`);
    for (const [k, c] of Object.entries(diff.envChanged))
        parts.push(`~env: ${k}: ${c.from} → ${c.to}`);
    const msg = `[AI override${override.note ? ' — ' + override.note : ''}] ${parts.join('; ') || '(no diff)'}`;
    if (channel && terminalHook)
        terminalHook(channel, msg, 'tips');
    if (auditHook) {
        auditHook({
            step: baseSpec.step,
...
function terminalForStep(step) {
    ...
    if (step === 'asm-pre' || step === 'asm')
        return 'tasm';
    if (step.startsWith('iverilog'))
        return 'tveri';
    if (step.startsWith('vvp'))
        return 'twave';
    if (step.startsWith('cocotb'))
        return 'twave';
    if (step.startsWith('verilator'))
        return 'twave';
    if (step === 'fst2vcd' || step === 'gtkwave')
        return 'twave';
    if (step === 'yosys-hierarchy')
        return 'twave';
    if (step === 'prism-yosys')
        return 'tveri';
    return null;
...
export async function runSpec(baseSpec, options = {}) {
    if (!baseSpec)
        throw new Error('runSpec: baseSpec is required');
    const shape = CommandSpec.validateShape(baseSpec);
    if (!shape.ok)
        throw new Error(`runSpec: invalid base spec: ${shape.error}`);
    const { appliedSpec, override, sources } = await applyResolved(baseSpec, { consumeEphemeral: !!options.consumeEphemeral });
...
export async function runSpecStreamed(baseSpec, options = {}) {
    if (!baseSpec)
        throw new Error('runSpecStreamed: baseSpec is required');
    const shape = CommandSpec.validateShape(baseSpec);
    if (!shape.ok)
        throw new Error(`runSpecStreamed: invalid base spec: ${shape.error}`);
    const { appliedSpec, override, sources } = await applyResolved(baseSpec, { consumeEphemeral: !!options.consumeEphemeral });
    logOverride(appliedSpec, baseSpec, override, sources, terminalForStep(baseSpec.step));
...
export async function resolveSpec(baseSpec, extraOverride = null) {
    ...
    if (extraOverride) {
        ...
        if (extraOverride.appendArgs)
            finalOverride.appendArgs = [...(finalOverride.appendArgs || []), ...extraOverride.appendArgs];
        if (extraOverride.prependArgs)
            finalOverride.prependArgs = [...(finalOverride.prependArgs || []), ...extraOverride.prependArgs];
        if (extraOverride.removeArgs)
            finalOverride.removeArgs = [...(finalOverride.removeArgs || []), ...extraOverride.removeArgs];
        if (extraOverride.envSet)
            finalOverride.envSet = { ...(finalOverride.envSet || {}), ...extraOverride.envSet };
        if (extraOverride.envUnset)
            finalOverride.envUnset = [...(finalOverride.envUnset || []), ...extraOverride.envUnset];
    }
    return {
        baseSpec,
