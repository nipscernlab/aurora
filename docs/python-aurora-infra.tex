...
Sem \texttt{pip} nem \texttt{ensurepip}, \texttt{venv} não é uma opção real; o
isolamento é por diretório mais \texttt{PYTHONPATH}. O diretório entra no
\texttt{AURORA\_COCOTB\_PYTHONPATH} pelo processo principal, em
\texttt{main/compile/executor.js}, e não onde o renderer monta a variável: assim
vale sempre, e o valor final é o nosso mesmo que um \emph{override} tenha mexido
na variável. Pela mesma razão, o \texttt{AURORA\_COCOTB\_PYTHONPATH} passou a
integrar as chaves protegidas do passo \texttt{cocotb-run} — depois que ele
carrega código instalado, reescrevê-lo passa a ser escolher de onde o Python
importa módulo.
