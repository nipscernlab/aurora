...
- [~] **A3** migrar globais (~482× electronAPI → imports) — PARCIAL 19/06/2026: criado `js/app/electron_api.js` (re-export) + 15 arquivos pequenos migrados (~26 sites). Resto (god-files compilation_module 226/tab_manager 27, aurora_api 33, médios, .ts) deferido pro A2. Ver §14.39. · [ ] **G8/G9** plugins + spawn único.
