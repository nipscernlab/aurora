...
/** Minimal scope shape accepted from either the VCD parser or the Verilog hierarchy tree. */
...
/** A signal enriched with its full dotted path (scope.path + '.' + name). */
type EnrichedSignal = ScopeSignal & { fullName: string };
/** A detected SAPHO processor instance. */
...
/** Per-processor translator file paths resolved under Temp/<procType>/. */
interface ProcPaths {
    tradOpcode: string | null;
    tradCmm: string | null;
    comp2gtkw: string | null;
}
/** Mutable global counters for inline file/proc filter IDs. */
interface FilterCounter {
    file: number;
    proc: number;
}
/** Options for {@link emitSignal}. */
interface EmitOpts {
    filter?: Set<string> | null;
    fileFilterPath?: string | null;
    procFilterPath?: string | null;
    counter?: FilterCounter;
}
/** Input for {@link buildAuroraGtkw}. */
interface BuildAuroraGtkwInput {
    vcdPath: string;
    gtkwPath: string;
    scopes: VcdScope[];
    tbModule?: string | null;
    tempBaseDir?: string | null;
    binDir?: string | null;
 ...
 */
export function resolveScopeModules(scopes: ScopeLike[], modules: Map<string, ModuleInfo>): Map<string, string | null> {
    const cache = new Map<string, string | null>();
    if (!modules || modules.size === 0 || !Array.isArray(scopes)) {
        return cache;
    }
    const resolve = (scopePath: string): string | null => {
        if (cache.has(scopePath)) return cache.get(scopePath) ?? null;
 ...
 * @returns {Set<string>}
 */
export function buildSignedSet(scopes: ScopeLike[], modules: Map<string, ModuleInfo>, scopeModules?: Map<string, string | null> | null): Set<string> {
    if (!modules || modules.size === 0 || !Array.isArray(scopes)) {
...
export function detectProcessors(scopes: ScopeLike[], scopeModules: Map<string, string | null> | null = null): ProcessorInstance[] {
    const found: ProcessorInstance[] = [];
...
function getScope(scopes: ScopeLike[], path: string): ScopeLike | null {
    return scopes.find((s) => s.path === path) || null;
}

function listSignalsInScope(scopes: ScopeLike[], scopePath: string): EnrichedSignal[] {
...
function findSignal(scopes: ScopeLike[], scopePath: string, name: string): EnrichedSignal | null {
    const scope = getScope(scopes, scopePath);
...
function emitComment(lines: string[], text: string): void {
    lines.push(`@${hex(FLAG_COMMENT_LINE)}`);
    lines.push(`-${text}`);
}

function emitGroupBegin(lines: string[], text: string): void {
    lines.push(`@${hex(FLAG_GROUP_BEGIN)}`);
    lines.push(`-${text}`);
}

function emitGroupEnd(lines: string[], text: string): void {
 ...
 */
function emitSignal(lines: string[], sig: EnrichedSignal | null, formatFlag: number, color: number, alias: string | null, opts: EmitOpts = {}): void {
    if (!sig) return;
...
function emitTopLevelSection(lines: string[], scopes: ScopeLike[], processorInstancePaths: string[], filter: Set<string> | null, signedSet: Set<string> | null): void {
    ...
    const emitList: Array<{ fullName: string; range: string | null }> = [];
    for (const scope of scopes) {
    ...
    const pickFlag = (sig: { fullName: string; range: string | null }): number => {
...
function emitIoSection(lines: string[], scopes: ScopeLike[], instancePath: string, filter: Set<string> | null): void {
...
function emitInstructionsSection(lines: string[], scopes: ScopeLike[], instancePath: string, paths: ProcPaths, counter: FilterCounter, _filter: Set<string> | null): void {
...
// pra montar o alias.
function findTypedVars(scopes: ScopeLike[], instancePath: string, prefix: string): Array<{ sig: EnrichedSignal; var: string; func: string }> {
    const sigs = listSignalsInScope(scopes, instancePath);
    const out: Array<{ sig: EnrichedSignal; var: string; func: string }> = [];
    for (const s of sigs) {
...
function emitArrayVars(lines: string[], scopes: ScopeLike[], instancePath: string, prefix: string, fmt: number, typeLabel: string, procFilterPath: string | null, counter: FilterCounter, filter: Set<string> | null): void {
    ...
    for (const s of sigs) {
        ...
        if (bucket) bucket.push({ sig: s, idx });
        else groups.set(base, [{ sig: s, idx }]);
    }
 ...
 */
export function buildAliasMap(scopes: ScopeLike[]): Map<string, string> {
...
function addProcessorAliases(map: Map<string, string>, scopes: ScopeLike[], proc: ProcessorInstance): void {
    ...
    for (const prefix of arrayPrefixes) {
        const groups = new Map<string, Array<{ sig: EnrichedSignal; idx: number }>>();
        for (const s of sigs) {
            if (!s.name.startsWith(prefix)) continue;
            const m = s.name.match(/^(.*?)(\d{4})$/);
            if (!m) continue;
...
export function buildAuroraGtkw({
    ...
    const resolveProcPaths = (proc: ProcessorInstance): ProcPaths => {
        const paths: ProcPaths = { tradOpcode: null, tradCmm: null, comp2gtkw: null };
        if (tempBaseDir) {
    ...
    // ou null.
    const findCoreOrTb = (corePath: string | null, name: string): EnrichedSignal | null => {
