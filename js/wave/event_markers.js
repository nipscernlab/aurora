...
export function hasIoEventSignals(scopes) {
    return scopes.some((s) => s.signals.some((sig) => /^req_in_sim_?\d+$/.test(sig.name) || /^out_en_sim_?\d+$/.test(sig.name)));
}
/**
...
export class EventScanner {
    ...
    inBody = false;
    time = 0;
    idKind = new Map(); // id VCD -> input/output (so req_in/out_en)
    inputTime = null;
    outputTime = null;
    linesSeen = 0;
    constructor(maxLines = 2_000_000) {
        this.maxLines = maxLines;
    }
    feed(chunk) {
        if (this.done() || this.capped() || !chunk)
            return;
        this.buf += chunk;
        let i;
    ...
    parseVar(t) {
        ...
        if (p.length < 5)
            return;
        const name = p[4];
        if (/^req_in_sim_?\d+$/.test(name))
            this.idKind.set(p[3], 'input');
        else if (/^out_en_sim_?\d+$/.test(name))
            this.idKind.set(p[3], 'output');
    }
    done() { return this.inputTime !== null && this.outputTime !== null; }
    capped() { return this.linesSeen >= this.maxLines; }
    /** Markers achados, em ordem (input primeiro). Vazio se nada encontrado. */
    markers() {
        const out = [];
        if (this.inputTime !== null)
            out.push({ time: this.inputTime, label: 'input' });
        if (this.outputTime !== null)
            out.push({ time: this.outputTime, label: 'output' });
        return out;
    }
}
