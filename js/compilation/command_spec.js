...
export function diffSpecs(before, after) {
    ...
    for (const k of Object.keys(envAfter)) {
        if (!(k in envBefore))
            envAdded[k] = envAfter[k];
        else if (envBefore[k] !== envAfter[k])
            envChanged[k] = { from: envBefore[k], to: envAfter[k] };
    }
...
export function validateShape(spec) {
    ...
    const s = spec;
    if (!STEP_IDS.includes(s.step))
        return { ok: false, error: `unknown step: ${String(s.step)}` };
    if (typeof s.binary !== 'string' || !s.binary)
        return { ok: false, error: 'binary must be a non-empty string' };
    if (!Array.isArray(s.args))
        return { ok: false, error: 'args must be an array' };
    for (const a of s.args) {
        if (typeof a !== 'string')
            return { ok: false, error: 'every arg must be a string' };
    }
