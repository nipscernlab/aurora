/**
 * compilation_helpers.js — funções PURAS de apoio à compilação, sem estado e
...
export function basenameOfPath(filePath) {
    return String(filePath || '').split(/[\\/]/).pop();
}

export function moduleStemFromPath(filePath) {
...
export function parseCocotbToplevelDirective(pySource) {
...
export function insertChegueiToaqui(src) {
    const s = String(src || '');
...
export function isVerilogLikeFile(filePath) {
    return /\.(v|sv|vh)$/i.test(String(filePath || ''));
}

export function assertPythonModuleName(filePath) {
