 ...
 */
function fimDoParen(texto, abre) {
    let nivel = 0;
 ...
 * consomem um `end`.
 */
function fimDoBegin(texto, inicio) {
 ...
 */
export function extractNamedGenerates(body) {
    const out = [];
...
export function parseIntLiteral(texto) {
    ...
    if (comBase) {
        const base = { b: 2, o: 8, d: 10, h: 16 }[comBase[1].toLowerCase()];
 ...
 */
export function declaredParams(header, body) {
    const out = new Map();
    const varrer = (texto) => {
 ...
 */
export function overriddenParams(paramList) {
    const out = new Map();
 ...
 */
export function resolveParamValue(texto, escopo) {
 ...
 */
export function evaluateCondition(condition, params, negated = false) {
