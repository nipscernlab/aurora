...
**Radical (8–10):**
- [~] **A2** decompor god-files (ai_assistant 4515 · compilation 4197 · tab_manager 2044) — EM ANDAMENTO. **compilation_module: decomposição COMPLETA (5/5 extrações)** 19/06/2026 — #1 hierarchy_parser +11 testes `eb14e3f`; #2 hierarchy_view `4eecdf7`; #3 wave_toolchain +16 testes `e03c07e`; #4 wave_signal_validator +13 testes `2a57e47`; #5 processor_compiler +18 testes (deps bag `_instanceDeps`, delegadores, seams `lastCompiledCmmPath`+`_chegueiInstrumentProc` preservados, revisão adversarial 0 defeitos). God-file **4197 → 3116 linhas (−1081)**, ~58 testes novos. Restante do A2: os god-files ai_assistant_manager (4515) e tab_manager (2044). Detalhe em §14.42.
