...
\texttt{build} do \tool{electron-builder} (\enquote{Aurora-IDE}): o primeiro
identifica o projeto npm; o segundo nomeia o aplicativo instalado.
...
\addlinespace
\texttt{download-norse-font.js} \newline (\file{Norse.otf})
...
A fonte \enquote{Norse} (\file{download-norse-font.js}) é caso à parte: a
licença permite \emph{embutir} a fonte no aplicativo, mas \textbf{proíbe
redistribuição}, então ela não é versionada --- é buscada da fonte original
(dafont) no \emph{bootstrap} e fica \emph{gitignored}; o \emph{build} a embute,
...
\midrule
\texttt{appId}        & \lstinline|com.nipscern.auroraide| \\
\texttt{productName}  & \texttt{Aurora-IDE} \\
...
\begin{enumerate}[nosep]
  ...
  \item \emph{download} da fonte Norse;
