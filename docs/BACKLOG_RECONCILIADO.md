...
- [~] **A3** migrar globais (~482× electronAPI → imports) — PARCIAL 19/06/2026: criado `js/app/electron_api.js` (re-export) + 15 arquivos pequenos migrados (~26 sites). Resto (god-files compilation_module 226/tab_manager 27, aurora_api 33, médios, .ts) deferido pro A2. Ver §14.39. · [x] **G8/G9** plugins + spawn único — FEITO 19/06/2026 (G9: `spawnTracked` único ponto de spawn + 9 sites de toolchain migrados, incl. decode-complex antes não-tracked; G8: política default-on×sob-demanda documentada). Ver §14.40.
...
**Radical (8–10):**
- [~] **A2** decompor god-files (ai_assistant 4592 · compilation 4197 · tab_manager 2044) — EM ANDAMENTO. **compilation_module: decomposição COMPLETA (5/5 extrações)** 19/06/2026 — #1 hierarchy_parser +11 testes `eb14e3f`; #2 hierarchy_view `4eecdf7`; #3 wave_toolchain +16 testes `e03c07e`; #4 wave_signal_validator +13 testes `2a57e47`; #5 processor_compiler +18 testes (deps bag `_instanceDeps`, delegadores, seams `lastCompiledCmmPath`+`_chegueiInstrumentProc` preservados, revisão adversarial 0 defeitos). God-file **4197 → 3116 linhas (−1081)**, ~58 testes novos. **ai_assistant_manager + tab_manager EM ANDAMENTO** (§14.43, plano mapeado por workflow + aprovado): Wave 1 (helpers puros) FEITA — AI-1 system_prompt `b654b4d`, TM-1 tab_utils `0b2eb53`, AI-2 chat_render (markdown+math+syntax+file-ref) `54722b2`, AI-3 ai_metadata `df357d0`. **Wave 2 iniciada: TM-2 file-type/ícone** (isImageFile/isPdfFile/isBinaryFile/getFileIcon + sets imageExtensions/pdfExtensions → tab_utils.js como funções puras; classe mantém 4 delegadores estáticos p/ os ~20 call sites internos + 4 callers externos de getFileIcon; byte-idêntico por script; +6 testes; tab_manager **1957→1834**). ai_assistant **4592→3376** (toda a região de helpers de módulo saiu; resta a classe); tab_manager **2044→1834**. +31 testes. **Restante:** classe do ai_assistant (streaming/provider/history/tool-chips — alto risco, ~40 campos) + tab_manager (file-type, file-ops, watchers, untitled, save_flow — estado estático entrelaçado) → **melhor com contexto fresco** (lição do compilation_module #3-5). Detalhe em §14.42/§14.43.
